    ...
    if 'Sender' not in df_filtered.columns or len(df_filtered) == 0:
        ...
        if sender_df.empty:
            ...
            if top_df is not None:
                ...
                # ---- Per-Staff KPIs ----
                source_df = df
                per_staff_df = dashboard_core.compute_per_staff_kpis(source_df)
                if per_staff_df is None or per_staff_df.empty:
                    st.info("No per-staff KPI data yet.")
                else:
                    st.dataframe(make_arrow_safe_df(per_staff_df), width="stretch", height=250)
